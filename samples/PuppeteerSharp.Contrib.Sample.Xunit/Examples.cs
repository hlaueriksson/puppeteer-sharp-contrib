...
namespace PuppeteerSharp.Documentation
{
    public class Examples : IAsyncLifetime
    {
        ...
        public async Task wait()
        {
            ...
            await _page.ClickAsync("#repository-container-header strong a");
        ...
        {
            await _page.GoToAsync("https://github.com/hardkoded/puppeteer-sharp");
            var element = await _page.QuerySelectorAsync("#repository-container-header strong a");
            ...
            Assert.Equal("<a data-pjax=\"#repo-content-pjax-container\" data-turbo-frame=\"repo-content-turbo-frame\" href=\"/hardkoded/puppeteer-sharp\">puppeteer-sharp</a>", outerHtml);
            ...
            outerHtml = await _page.EvaluateExpressionAsync<string>($"document.querySelector({"'#repository-container-header strong a'"}).outerHTML");
            innerText = await _page.EvaluateExpressionAsync<string>($"document.querySelector({"'#repository-container-header strong a'"}).innerText");
            url = await _page.EvaluateExpressionAsync<string>($"document.querySelector({"'#repository-container-header strong a'"}).getAttribute('href')");
            hasContent = await _page.EvaluateExpressionAsync<bool>($"document.querySelector({"'#repository-container-header strong a'"}).textContent.includes({"'puppeteer-sharp'"})");
            Assert.Equal("<a data-pjax=\"#repo-content-pjax-container\" data-turbo-frame=\"repo-content-turbo-frame\" href=\"/hardkoded/puppeteer-sharp\">puppeteer-sharp</a>", outerHtml);
            Assert.Equal("puppeteer-sharp", innerText);
            Assert.Equal("/hardkoded/puppeteer-sharp", url);
            Assert.True(hasContent);
